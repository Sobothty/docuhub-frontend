...
function ChartTooltipContent({
  ...
  const safePayload: TooltipPayloadItem[] = Array.isArray(payload)
      ? payload.filter((item): item is TooltipPayloadItem => typeof item === "object" && item !== null)
      : []
      ...
      >
        ...
        <div className="grid gap-1.5">
          {safePayload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            // Only access item.payload if it exists and is an object
...
function getPayloadConfigFromPayload(
  ...
  // Safe check for 'payload' property
  ...
  if (
    ...
    payloadPayload = (payload as { payload?: Record<string, unknown> }).payload
  }

  let configLabelKey: string = key

  // Safe check for 'label' property
  if (
